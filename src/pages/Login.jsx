        ...
        if (Array.isArray(key)) {
            key.forEach(k => {
                if (validationErrors[k]) {
                    return Array.isArray(validationErrors[k]) ? validationErrors[k][0] : validationErrors[k];
                }
            })
            return '';
        } else {
            ...
            <Box sx={{ px: 8, py: 4, border: '1px solid rgba(0,0,0,0.15)', borderRadius: '8px', width: '15%' }}>
                ...
                >
                    <TextField
                        ...
                        helperText={getValidationErrorText('email') || validationErrors['detail']}
                        value={email}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mt: 3 }}>
                    ...
                    <Link to={"/registration"}>
                        ...
                        <Typography variant="caption" sx={{ fontSize: '10px' }} display="block" gutterBottom>
                            Don't have account?
                        </Typography>
                    </Link>
                </Box>
